...
public class EmployeeFactoryTest {
    ...
    public void createEmployee_shouldReturnSalariedEmployee_whenTypeSE() {
        ...
        assertEquals(SalariedEmployee.class, emp.getClass()); //exactly type, not is-a, match
        assertEquals("Jackie", emp.getName());
        // name and hire date can be combined into a separate test method because they are repeated

        assertEquals(Date.valueOf("1990-08-24"), emp.getHireDate());

        //downcast emp to more specific type SalariedEmployee
        SalariedEmployee semp = (SalariedEmployee) emp;
        assertEquals(50000, semp.getSalary(), .001);
    }
    ...
    public void createEmployee_throwsIllegalArgumentException_typeInvalid() {
